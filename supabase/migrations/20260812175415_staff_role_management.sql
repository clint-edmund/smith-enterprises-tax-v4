...
begin
  ...
  if current_profile.role <> 'administrator'::public.app_role then
    raise exception 'You are not authorized to manage staff roles.';
  end if;
  ...
  if target_profile.id is null then
    raise exception 'Target staff account was not found.';
  end if;

  if target_staff_id = auth.uid()
     and new_role <> 'administrator'::public.app_role then
...
comment on function public.update_staff_role(uuid, public.app_role)
is
  'Allows active administrators to change another staff member role while preventing self-demotion.';

revoke all
on function public.update_staff_role(uuid, public.app_role)
from public;
